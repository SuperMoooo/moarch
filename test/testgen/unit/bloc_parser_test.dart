...
import 'package:moarch/src/testgen/unit/analyzers/event_parser.dart';
import 'package:moarch/src/testgen/unit/analyzers/feature_scanner.dart';
import 'package:moarch/src/testgen/unit/analyzers/notifier_parser.dart';
...
void main() {
    ...
    test(
      ...
      () {
        ...
        try {
          ...
          expect(
            bundles.map((b) => b.featureName),
            equals(['auth', 'cart', 'user']),
          );

          final auth = bundles.first;
          expect(
            auth.notifierFiles.map(p.basename),
          ...
          expect(
            user.notifierFiles.map(p.basename),
            equals(['user_notifier.dart']),
          );
  ...
  group('NotifierParser — flutter_bloc', () {
    test('detects a Bloc: kind, event base type, state type, registered events '
        'and its constructor-injected dependencies', () {
      final tempDir = Directory.systemTemp.createTempSync('mogen_bloc_');
      try {
        final file = File(
          '${tempDir.path}${Platform.pathSeparator}auth_bloc.dart',
        );
...
''');
    ...
    test('detects a Cubit: its public methods are the actions, and named '
        'constructor dependencies keep their parameter name', () {
      final tempDir = Directory.systemTemp.createTempSync('mogen_bloc_');
      try {
        final file = File(
          '${tempDir.path}${Platform.pathSeparator}cart_cubit.dart',
        );
        file.writeAsStringSync('''
...
''');
        ...
        expect(
          cubit.repositories.map((r) => r.type),
          equals(['CartRepository']),
        );
        expect(
          cubit.constructorParams.map((p) => p.name),
...
''');
  ...
  group('EventParser', () {
    test('resolves `required this.x` constructor parameters back to their '
        'declared field types, and flags part files', () {
      final tempDir = Directory.systemTemp.createTempSync('mogen_event_');
      try {
        final file = File(
          '${tempDir.path}${Platform.pathSeparator}auth_event.dart',
        );
        file.writeAsStringSync('''
