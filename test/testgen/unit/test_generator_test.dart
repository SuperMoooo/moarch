...
import 'package:moarch/src/testgen/unit/generators/test_generator.dart';
import 'package:moarch/src/testgen/unit/models/notifier_info.dart';
...
void main() {
  group('TestGenerator', () {
    ...
    test('generates imports for every repository dependency, including ones '
    ...
    test('filters out service/non-repository types (LocalAuth)', () {
      ...
      expect(output, isNot(contains('localAuthProvider')));
    });
    ...
    test('generates state field assertions for known patterns', () {
      ...
      try {
        final file = File(
          '${tempDir.path}${Platform.pathSeparator}form_notifier.dart',
        );
...
''');
    ...
    test('leaves out the state import when nothing in the test names it', () {
      const notifier = NotifierInfo(
      ...
      // Assertions read fields through `finalState.requireValue`, so an
      // import of the state class would be unused_import in the project.
      expect(
        output,
        isNot(
          contains(
...
''');
    ...
    test('omits the error-path test when the method makes no dependency '
      ...
      expect(
        output,
        isNot(contains('shows an error when the repository fails')),
      );
...
''');
    ...
    test('mocks a cross-notifier dependency and resolves its import via the '
    ...
    test('mocks a raw SDK dependency reached via ref.read even though its '
      ...
      expect(output, contains('TODO(moarch)'));
    });
    ...
    test('resolves a dependency import from the notifier\'s own source file', () {
      const notifier = NotifierInfo(
        ...
        buildMethod: null,
        methods: [],
        sourceImports: ['package:app/core/network/api_client.dart'],
      );
      ...
      expect(output, isNot(contains('TODO(moarch)')));
    });
...
''');
    ...
    test('mocks a router dependency as GoRouter from go_router', () {
      ...
      expect(output, contains("import 'package:go_router/go_router.dart';"));
      expect(output, isNot(contains('TODO(moarch)')));
      expect(output, isNot(contains('implements Router')));
    });

    test('adds the interface import alongside a source-discovered impl-only '
    ...
    test('resolves the interface import from the dependency\'s own feature, '
    ...
    test('strips .future from a captured provider expression before using it '
    ...
    test('detects a direct call through a private underscore-prefixed field '
...
''');
    ...
    test('detects a call through an abbreviated field name that does not '
...
''');
        ...
        final notifier = NotifierInfo(
          ...
          methods: const [
            MethodInfo(
              ...
              returnType: 'Future<void>',
              isAsync: true,
              params: [ParamInfo(name: 'habitId', type: 'int', isNamed: true)],
            ),
          ],
        );
    ...
    test('does not stub a method on dependencies that method never actually '
...
''');
    ...
    test('stubs build()\'s dependency calls once in setUp(), after the '
...
''');
        ...
        // succeeds).
        final errorTest = loginSection.substring(
          loginSection.indexOf("test('login shows an error"),
        );
        ...
        expect(
          ...
          lessThan(
            errorTest.indexOf(
              "thenThrow(const ServerException(message: 'test'))",
            ),
          ),
          reason:
              'the notifier must initialise with the success stubs still '
    ...
    test('registers mocktail fallback values in setUpAll for custom-typed '
...
''');
        ...
        final notifierFile = File(
          '${tempDir.path}${Platform.pathSeparator}cart_notifier.dart',
        );
...
''');
    ...
    test('mocks a notifier dependency from the registry by extending its real '
    ...
    test('reads a family notifier with its argument everywhere: declaration, '
    ...
    test('strips family call arguments from a captured dependency provider '
    ...
    test('hoists a stub shared by two or more methods to the end of setUp() '
        'instead of repeating it in every success test', () {
      final tempDir = Directory.systemTemp.createTempSync('mogen_test_');
      try {
        final file = File(
          '${tempDir.path}${Platform.pathSeparator}sync_notifier.dart',
        );
        file.writeAsStringSync('''
...
''');
        ...
        expect(
          successA,
          isNot(contains('when(() => mockDataRepository.sync(')),
        );
    ...
    test('asserts state fields directly (no requireValue) for a synchronous '
...
''');
    ...
    test('skips the success-message assertion when the method body provably '
        'never sets one', () {
      const notifier = NotifierInfo(
