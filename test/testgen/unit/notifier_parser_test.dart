...
void main() {
  group('NotifierParser', () {
    test(
      'resolves routerProvider to GoRouter, not the guessed Router type',
      () {
...
''');
          ...
          final router = notifiers.first.repositories.firstWhere(
            (r) => r.providerExpression == 'routerProvider',
          );
    ...
    test('detects a family notifier: family flag, state type and argument '
        'type from the generics, and the full superclass source', () {
      final tempDir = Directory.systemTemp.createTempSync('mogen_parser_');
      try {
        final file = File(
          '${tempDir.path}${Platform.pathSeparator}chat_notifier.dart',
        );
        file.writeAsStringSync('''
...
''');
    ...
    test('ignores classes extending ChangeNotifier/ValueNotifier even when '
...
''');
    ...
    test('marks a method that returns a Future without the async keyword as '
...
''');
        ...
        final methods = {for (final m in notifiers.first.methods) m.name: m};
