...
import 'package:moarch/src/testgen/unit/generators/bloc_test_generator.dart';
...
NotifierInfo _authBloc(String sourcePath) => NotifierInfo(
  ...
  events: const [
    EventInfo(
      ...
      isAsync: true,
      handlerBodySource:
          '{ try { await _authRepository.login(); } '
...
void main() {
  group('BlocTestGenerator', () {
    ...
    test('drives a bloc with bloc_test: a blocTest per event, built from the '
...
''');
        ...
        expect(
          output,
          contains('AuthBloc buildAuthBloc() => AuthBloc(mockAuthRepository);'),
        );
    ...
    test('stubs only the calls the event\'s own handler makes, keeping other '
...
''');
    ...
    test('hoists the calls the constructor itself makes into setUp(), and does '
...
''');
        ...
        final errorTest = output.substring(
          output.indexOf('surfaces an error when a dependency'),
        );
    ...
    test('asserts bloc errors instead of state when the handler never catches '
...
''');
        ...
        expect(
          errorTest,
          isNot(contains('expect(bloc.state.error, isNotNull)')),
        );
      } finally {
        tempDir.deleteSync(recursive: true);
      }
...
''');
          ...
          NotifierInfo(
            ...
            methods: const [
              MethodInfo(
                ...
                bodySource:
                    '{ try { await _cartRepository.addItem(itemId: '
    ...
    test('mocks a bloc dependency with bloc_test\'s MockBloc and stubs its '
...
''');
        ...
        final output = registryGenerator.generate(
          NotifierInfo(
    ...
    test('builds the event with real constructor arguments resolved from the '
...
''');
    ...
    test('asserts a sealed state by the type the handler emits, and offers the '
...
''');
    ...
    test('asserts the emitted subtype AND the shared fields when a state does '
...
''');
        ...
        expect(
          successTest,
          contains('expect(bloc.state, isA<ProfileLoaded>());'),
        );
        ...
        expect(output, contains('expect(bloc.state, isA<ProfileInitial>());'));
    ...
    test('does not read fields off the state when the matched state class is '
...
''');
    ...
    test('still falls back to a TODO when nothing in the handler names a '
        'concrete state type', () {
      final tempDir = Directory.systemTemp.createTempSync('mogen_bloc_gen_');
      try {
        final file = _write(tempDir, 'auth_bloc.dart', '''
...
''');
        ...
        expect(output, contains('TODO(moarch): assert the resulting state'));
