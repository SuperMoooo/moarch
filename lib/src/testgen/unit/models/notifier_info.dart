...
class EventClassInfo {
  ...
  /// Whether that constructor is `const` — an instance built from literal
  /// arguments is then written `const X(...)`, which prefer_const_constructors
  /// asks for in the generated project.
...
class RepositoryDep {
  ...
  /// Whether the dependency is read out of the get_it locator —
  /// `getIt<AuthRepository>()`, usually behind a getter — rather than through
  /// a provider or the constructor. That is how a moarch Riverpod notifier
  /// reaches its repositories, so the test registers the mock in `getIt`
  /// instead of overriding a provider.
  final bool viaLocator;
