...
class BlocTestGenerator {
  ...
  String generate(NotifierInfo n) {
    ...
    _imports(
      ...
      plan,
      needsAppException:
          body.contains('ServerException(') || body.contains('AppException'),
    );
    buf.write(body);
  ...
  _BlocPlan _buildPlan(NotifierInfo n) {
    ...
    for (final action in actions) {
      for (final dep in dependencies) {
        for (final call
            in callsByAction[action.name]![dep.type] ??
    ...
    for (final dep in dependencies) {
      ...
      final leaf = MockValueGenerator.extractCustomType(
        depInfo.stateType ?? '',
      );
      if (leaf == null) continue;
      final stateImport = depInfo.stateInfo;
      imports.add(
        stateImport != null && !stateImport.isPart
  ...
  void _imports(
    ...
    b.writeln(TestGenConventions.header);
    b.writeln();
  ...
  void _mocks(StringBuffer b, NotifierInfo n, _BlocPlan plan) {
    ...
    for (final dep in plan.dependencies) {
      ...
      if (depInfo != null && depInfo.isBloc) {
        ...
        b.writeln(
          'class Mock${dep.type} extends MockBloc<$event, $state> '
          'implements ${dep.type} {}',
        );
      } else if (depInfo != null && depInfo.isCubit) {
        final state = depInfo.stateType ?? 'dynamic';
        b.writeln(
          'class Mock${dep.type} extends MockCubit<$state> '
          'implements ${dep.type} {}',
        );
      } else {
        b.writeln(
  ...
  void _setUp(StringBuffer b, NotifierInfo n, _BlocPlan plan) {
    ...
    if (blocDeps.isNotEmpty) {
      ...
      for (final dep in blocDeps) {
        final depInfo = notifierRegistry[dep.type]!;
        final stateVal = MockValueGenerator.forReturnType(
          depInfo.stateType ?? 'dynamic',
        );
  ...
  void _actionTests(
    ...
    final hasDependencyCalls = plan.dependencies.any(
      (dep) =>
    ...
    if (_actionOwnCalls(action, plan).isNotEmpty) {
      ...
      b.writeln(
        '      // No error-path test: every dependency call '
        '${action.name} makes is',
      );
      b.writeln(
        '      // also made by the constructor, so failing it would '
        'break build()',
      );
  ...
  bool _catchesErrors(_BlocAction action) {
    ...
    // moarch's `runAction` (ActionBlocMixin) catches the AppException and
    // writes it to the state's errorMessage — the handler has no catch of
    // its own to find.
    return body.contains('catch') ||
        body.contains('onError') ||
        body.contains('runAction');
  }
  ...
  void _writeExpectScaffold(
    ...
    final sequence = action.emits
        ...
        .toList();
    final namesAConcreteType = action.emits.any(
      (e) => e.typeName != null && e.typeName != n.stateType,
    );
  ...
  void _writeStubSetUp(
    ...
    for (final (dep, call) in entries) {
      if (throwing) {
        ...
        b.writeln(
          '              .thenThrow(${TestGenConventions.errorValue});',
        );
      } else {
        StubWriter.writeSuccessStub(
          b,
  ...
  void _writeStateTodo(StringBuffer b, NotifierInfo n, String sut) {
    b.writeln('          // TODO(moarch): assert the resulting state, e.g.');
    b.writeln(
  ...
  String _actExpression(_BlocAction action, NotifierInfo n) {
    if (action.isEvent) {
      return 'bloc.add(${_construct(action.name, action.params)})';
    }
  ...
  /// Builds `ClassName(args)` for an event or a state from the registry,
  /// skipping named parameters that already declare a default — and prefixed
  /// `const` when the class allows it and every argument is a literal, which
  /// is what prefer_const_constructors asks for in the generated project.
  String _construct(String className, List<ParamInfo> params) {
    final passed = params
    ...
    final args = passed
        .map(
          (p) =>
              p.isNamed ? '${p.name}: ${_literal(p.type)}' : _literal(p.type),
        )
        .join(', ');
    final isConst =
        (eventRegistry[className]?.hasConstConstructor ?? false) &&
        passed.every((p) => StubWriter.isConstCompatible(p.type));
    return '${isConst ? 'const ' : ''}$className($args)';
  }

  /// A literal for [type] with any leading `const` dropped — the whole
  /// construction carries it instead.
  String _literal(String type) {
    final value = MockValueGenerator.forType(type);
    return value.startsWith('const ') ? value.substring(6) : value;
  }

  /// The state a guarded handler needs to start from, as a `seed:` value.
  ///
  /// A handler that only acts in one state bails out first —
  /// `if (current is! AuthAuthenticated) return;` — and a bloc built for a
  /// test starts in its initial state, so without a seed the test verifies an
  /// action that never ran. `null` when there is no such guard, or the state
  /// class is not one the project scan found.
  String? _seed(_BlocAction action) {
    final body = action.bodySource;
    if (body == null) return null;
    final match = RegExp(
      r'\bis!\s+(\w+)\s*\)\s*\{?\s*return\b',
    ).firstMatch(body);
    if (match == null) return null;
    final type = match.group(1)!;
    final info = eventRegistry[type];
    if (info == null) return null;
    return _construct(type, info.params);
  }

  void _writeSeed(StringBuffer b, _BlocAction action) {
    final seed = _seed(action);
    if (seed == null) return;
    b.writeln('        // The handler only acts from this state.');
    b.writeln('        seed: () => $seed,');
  }
