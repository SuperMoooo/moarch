...
class TestGenerator {
  ...
  String generate(NotifierInfo notifier) {
    ...
    _imports(
      ...
      notifier,
      plan,
      needsAppException: body.contains('ServerException('),
      body: body,
    );
    buf.write(body);
  ...
  _Plan _buildPlan(NotifierInfo n) {
    ...
    for (final methodName in methodNames) {
      for (final repo in dependencies) {
        for (final call
            in callsByMethod[methodName]![repo.type] ??
    ...
    for (final repo in dependencies) {
      ...
      final stateLeaf = MockValueGenerator.extractCustomType(
        dep.stateType ?? '',
      );
      if (stateLeaf != null) {
    ...
    for (final repo in dependencies) {
      ...
      if (buildName != null) {
        for (final call
            in callsByMethod[buildName]![repo.type] ??
                const <RepositoryMethodCall>[]) {
      ...
      for (final method in publicMethods) {
        ...
        for (final call
            in callsByMethod[method.name]![repo.type] ??
                const <RepositoryMethodCall>[]) {
  ...
  void _imports(
    ...
    // The locator, when a dependency is registered in it.
    if (plan.dependencies.any((r) => r.viaLocator)) {
      lines.add(TestGenConventions.injectorImport(n.packageName));
    }

    // Notifier
    lines.add("import '${n.importPath}';");

    // The state, when the test names it — assertions read its fields through
    // `finalState.requireValue`, which needs no import of its own.
    final stateInfo = n.stateInfo;
    if (stateInfo != null &&
        RegExp('\\b${stateInfo.className}\\b').hasMatch(body)) {
  ...
  void _mocks(StringBuffer b, NotifierInfo n, _Plan plan) {
    ...
    for (final repo in plan.dependencies) {
      ...
      if (dep?.superclassSource != null) {
        ...
        b.writeln(
          'class Mock${repo.type} extends ${dep!.superclassSource} '
  ...
  void _setUp(StringBuffer b, NotifierInfo n, _Plan plan) {
    ...
    // The locator is global, so each test registers its own mocks and
    // tearDown resets it.
    final locatorDeps = plan.dependencies.where((r) => r.viaLocator).toList();
    if (locatorDeps.isNotEmpty) {
      b.writeln();
      b.writeln('      // The notifier reads these out of getIt.');
      for (final repo in locatorDeps) {
        b.writeln(
          '      getIt.registerSingleton<${repo.type}>'
          '(mock${repo.type});',
        );
      }
    }
    final providerDeps = plan.dependencies.where((r) => !r.viaLocator).toList();

    b.writeln();
    b.writeln('      container = ProviderContainer(');

    if (providerDeps.isNotEmpty) {
      b.writeln('        overrides: [');
      for (final repo in providerDeps) {
        final rawProvider =
  ...
  void _tearDown(StringBuffer b, {required bool resetLocator}) {
    if (resetLocator) {
      b.writeln('    tearDown(() async {');
      b.writeln('      container.dispose();');
      b.writeln('      await getIt.reset();');
      b.writeln('    });');
    } else {
      b.writeln('    tearDown(() {');
      b.writeln('      container.dispose();');
  ...
  void _methodErrorTest(
    ...
    for (final repo in plan.dependencies) {
      ...
      for (final call in calls) {
        ...
        b.writeln('            .thenThrow(${TestGenConventions.errorValue});');
      }
    }
  ...
  void _stubSuccessCalls(
    ...
    for (final repo in plan.dependencies) {
      ...
      final remaining = calls
          .where((c) => !hoistedNames.contains(c.methodName))
          .toList();

      if (calls.isEmpty) {
        b.writeln('        // No mocks needed for ${repo.type}');
      } else if (remaining.isEmpty) {
        final allFromBuild = calls.every(
          (c) =>
  ...
  String _buildArgList(List<ParamInfo> params) => params
      .map((p) {
