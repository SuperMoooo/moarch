...
class ImportResolver {
  ...
  List<String> dependencyImports(RepositoryDep repo, NotifierInfo n) {
    ...
    if (isRepositorySuffix(repo.type)) {
      ...
      final featureName =
          findActualFeatureFolder(snakeCase) ??
    ...
    return [
      '// TODO(moarch): could not resolve an import for '
  ...
  String toPortableImport(String uri, NotifierInfo n) {
    ...
    if (p.isWithin(libPath, absoluteTarget)) {
      final rel = p
          .relative(absoluteTarget, from: libPath)
          .replaceAll(r'\', '/');
...
class StubWriter {
  ...
  static void writeSuccessStub(
    ...
    if (type != null && _isFutureOfVoid(type)) {
      // `async => null` for a Future<void> is what avoid_returning_null_for_void
      // flags in the generated project; an empty async body is the same value.
      b.writeln('$indent    .thenAnswer((_) async {});');
    } else if (type == null) {
  ...
  static bool _isFutureOfVoid(String type) {
    final compact = type.replaceAll(' ', '');
    return compact == 'Future<void>' || compact == 'FutureOr<void>';
  }
  ...
  static String inputDeclarationLine(ParamInfo param) {
    ...
    final cleanedVal = val.startsWith('const ')
        ? val.substring('const '.length)
        : val;
...
class DependencySelector {
  ...
  static List<RepositoryDep> mockable(NotifierInfo n) {
    ...
    return n.repositories
        .where(
          (r) =>
              r.providerExpression != null ||
              r.viaLocator ||
              looksLikeMockableName(r.type),
        )
        .toList();
  }
