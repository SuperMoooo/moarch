...
class MethodCallDetector {
  ...
  static String? resolveReturnType(String repoSourcePath, String methodName) =>
...
class _MethodCallVisitor extends RecursiveAstVisitor<void> {
  ...
  /// Checks if this is an inline locator call —
  /// `getIt<AuthRepository>().login(...)`.
  bool _isLocatorCall(MethodInvocation node) {
    final target = node.target;
    if (target is! MethodInvocation) return false;
    final typeArgs = target.typeArguments?.arguments;
    if (typeArgs == null || typeArgs.length != 1) return false;
    final name = target.methodName.name;
    final owner = target.target?.toSource();
    final isLookup =
        (owner == null && (name == 'getIt' || name == 'sl')) ||
        ((owner == 'getIt' || owner == 'sl') && name == 'get');
    return isLookup && typeArgs.first.toSource() == repoType;
  }
  ...
  bool _providerMatchesRepo(String providerExpr) {
    ...
    final typeFromProvider = ProviderTypeResolver.typeFromProviderExpression(
      providerExpr,
    );
