...
class MockValueGenerator {
  ...
  static String forReturnType(String rawType) {
    ...
    switch (innerClean) {
      ...
      case 'num':
        return '0';
      case 'bool':
        // A dependency's bool is almost always a gate — `isLoggedIn()`,
        // `exists()`, `canRetry()` — and `true` is the answer that lets the
        // action carry on to the rest of its calls, which the success test
        // then verifies. `false` takes the early exit and verifies nothing.
        return 'true';
      case 'dynamic':
      case 'Object':
        return 'null';
  ...
  static String? extractCustomType(String rawType) {
    ...
    // Unwrap List<T>
    final candidate = innerClean.startsWith('List<')
        ? _generic(innerClean)
        : innerClean;
