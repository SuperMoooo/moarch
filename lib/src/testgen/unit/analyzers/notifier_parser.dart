...
class _RepoRefVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    final locatorType = locatorTypeOf(node);
    if (locatorType != null) {
      if (repos.any((r) => r.type == locatorType)) return;
      repos.add(
        RepositoryDep(
          type: locatorType,
          name: _holderName(node) ?? _typeToFieldName(locatorType),
          viaLocator: true,
        ),
      );
      return;
  ...
  /// The type a get_it lookup resolves — `getIt<AuthRepository>()` or
  /// `getIt.get<AuthRepository>()` — or `null` when [node] is neither.
  static String? locatorTypeOf(MethodInvocation node) {
    final typeArgs = node.typeArguments?.arguments;
    if (typeArgs == null || typeArgs.length != 1) return null;
    final target = node.target?.toSource();
    final method = node.methodName.name;
    final isCall = target == null && (method == 'getIt' || method == 'sl');
    final isGet =
        (target == 'getIt' || target == 'sl' || target == 'GetIt.instance') &&
        method == 'get';
    if (!isCall && !isGet) return null;
    return typeArgs.first.toSource().replaceAll('?', '').trim();
  }

  /// The member the lookup is stored behind, so calls through it can be
  /// matched: the getter in `AuthRepository get _repo => getIt<...>()`, or
  /// the field in `late final _repo = getIt<...>()`.
  String? _holderName(MethodInvocation node) {
    AstNode? current = node.parent;
    while (current != null) {
      if (current is MethodDeclaration) {
        return current.isGetter ? current.name.lexeme : null;
      }
      if (current is VariableDeclaration) return current.name.lexeme;
      if (current is ClassDeclaration) return null;
