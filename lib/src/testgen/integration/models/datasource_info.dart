...
class EndpointInfo {
  ...
  /// The generated argument list for the datasource constructor call.
  final String constructorArguments;

  /// Generates the test file name from the endpoint's group and name.
  ///
  /// Both segments are included so endpoints with the same trailing path
  /// (e.g. `/Users/list` and `/Products/list`) do not collide.
  ///
  /// Example: endpoint `/Authenticate/Auth` → 'authenticate_auth_integration_test.dart'
  String get fileName {
    final groupPart = _toSnakeCase(group);
    final namePart = _toSnakeCase(name);
    final segment = namePart.isEmpty || namePart == groupPart
        ? groupPart
        : '${groupPart}_$namePart';
    return '${segment}_integration_test.dart';
  }
