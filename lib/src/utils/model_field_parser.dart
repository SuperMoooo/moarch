...
class ModelFieldParser {
  ...
  ///
  /// Callers use it to stay inside the one class they mean. A model file is
...
/// A single constructor field extracted from a model class.
class ModelField {
  ...
  ///
  /// Carried so a generated member keeps the field's own defaults and keys
  /// instead of turning every field into a required one.
  final List<String> annotations;

  /// The parameter as it is written in a freezed factory.
  String get asParameter {
    final prefix = annotations.isEmpty ? '' : '${annotations.join(' ')} ';
    return '$prefix${isRequired ? 'required ' : ''}${type ?? 'dynamic'} $name';
  }
}
