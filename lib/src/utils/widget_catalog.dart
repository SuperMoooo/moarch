...
class WidgetSpec {
  ...
  /// Path relative to the widget's [dir], e.g. `inputs/app_switch.dart`.
  final String file;

  /// The directory under `lib/shared/` this entry is written into.
  ///
  /// `widgets` for the kit itself, and `views` for the one entry that is a
  /// route rather than a piece of the kit. The generated shape already draws
  /// that line — a feature's screens live in `presentation/views/` — so a
  /// screen belonging to no feature is a `shared/views/` file, not a widget.
  final String dir;

  /// Where moarch used to write this file, relative to `lib/`, for an entry
  /// that has since moved.
  ///
  /// Null for everything that has always been where it is. `update` reads it
  /// to recognise a project still holding the file at the old path, so the
  /// refresh relocates it instead of writing a second copy and leaving the
  /// first behind — see [UpdateCommand].
  final String? movedFrom;

  /// Path relative to `lib/`, forward-slash form: what the manifest keys on
  /// and what the CLI prints.
  String get libFile => 'shared/$dir/$file';

  /// Absolute path to this entry inside a project's [libPath].
  String pathIn(String libPath) => p.join(libPath, 'shared', dir, file);

  /// Absolute path to the pre-move location inside [libPath], or null when
  /// this entry never moved.
  String? legacyPathIn(String libPath) {
    final from = movedFrom;
    return from == null ? null : p.join(libPath, p.joinAll(from.split('/')));
  }
...
abstract final class WidgetCatalog {
  ...
  /// the whole kit, and that stays true as widgets are added. It is also the
  /// one entry that is not a widget: it consumes the kit rather than
  /// belonging to it, which is why it is the only `shared/views/` file here.
  static final List<WidgetSpec> all = [
    ...
    WidgetSpec(
      ...
      dir: 'views',
      movedFrom: 'shared/widgets/design_system_view.dart',
  ...
  static String markdown() {
    ...
    for (final category in categories) {
      ...
      for (final w in items) {
        ...
        if (w.dir != 'widgets') notes.add('_lib/shared/${w.dir}/_');
        buffer.writeln(
