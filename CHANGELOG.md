...
- moarch needs **Dart 3.9+** (Flutter 3.35+). The test generator below reads
  source with `analyzer` (10–14 supported), which needs 3.9.
- `init` no longer adds `mogen_unit_tests` / `mogen_integration_tests` to the
...
- Writes `presentation/scopes/<name>_scope.dart`:
  - `of(context)` collects the blocs;
  - `provide(child:)` provides the same instances again;
  - `context.show<Name>Sheet` / `show<Name>Dialog` do both.
- The blocs default to every one the feature declares; `--blocs` picks them
  from any feature.
- `--parent XScope` nests scopes: the outer scope's blocs are provided around
  this one's.
- Prints a `ShellRoute` for nested routes, which survives a deep link where
  `extra` does not.
- On Riverpod it explains why there is nothing to generate.
- The generated `AGENTS.md` tells agents to use it rather than create a second
  instance of a bloc.

**`moarch create tests [feature]`** — the two mogen packages, merged in and
adapted to a moarch project:

- **Unit:** a test per notifier, bloc and cubit under `test/unit/features/`.
  - A notifier's `getIt<X>()` dependencies (behind a getter, or inline) are
    ...
    notifier uses.
  - A bloc handler that goes through `runAction` has its failure asserted on
    `errorMessage`.
  - A handler guarded by `if (state is! X) return;` gets `X` as its `seed:`.
  - Bool stubs answer `true`, so the success path reaches the rest of the
    handler.
  - Error paths throw `ServerException` instead of a test-only
    `AppException.test()`.
  - Events with const constructors are added as `const`.
  - The state import is written only when the test names the state.
- **Integration:** a test per GET endpoint under `test/integration/features/`.
  - `ApiConstants.x` paths are resolved (mogen only read literals).
  - Tests run on `flutter_test`, not `package:test`.
  - Only a `NetworkException` fails a test, so `safeApiCall`'s mapping is
    respected.
  - `dio_helper.dart` is built from `AppEnv.baseUrl` and `ApiConstants`, and is
    never overwritten.
- **Both:**
  - A `GENERATED BY moarch` marker, and a file without it is left alone
    unless `--force`. Files mogen wrote are still refreshed.
  - `--dry-run`, `--no-unit` / `--no-integration`, `--verbose`.
  - Adds the missing test packages.
  - Warns about exactly the models whose `.empty()` the tests call and that
    lack it.
- The generator runs inside the globally-activated CLI, so `analyzer` no
...
- Generated tests analyze clean and pass on fresh Riverpod and bloc projects
  (Flutter 3.44).
...
- **`AGENTS.md` and `CLAUDE.md`.** `init` writes the project's rules for coding
  ...
  is only `@AGENTS.md`. Both are Docs catalog entries (`moarch update agents
  claude-md`), and `moarch doctor --fix` adds them to a project scaffolded
  before this release.
...
- **FCM in the background actually shows data-only pushes.** The generated
  background handler used to only log. A background message runs in its own
  isolate, with no Firebase app, no locator and no initialized notifications
  plugin, so a data-only push arrived and nothing showed. With the local
  notifications service also generated, the handler now sets up exactly what it
  needs in that isolate (idempotently) and shows the message. Foreground
  messages the OS will not display (any message on Android, data-only on iOS)
  are shown too. The data goes along as the payload.
